...
from face_swap.face_detector.mtcnn_pytorch.src.get_nets import PNet, RNet, ONet
from face_swap.face_detector.mtcnn_pytorch.src.box_utils import (
    nms,
    calibrate_box,
    get_image_boxes,
    convert_to_square,
)
from face_swap.face_detector.mtcnn_pytorch.src.first_stage import run_first_stage
from face_swap.face_detector.mtcnn_pytorch.src.align_trans import (
...
class MTCNN:
    ...
    def align_fully(
        ...
        for i in ori:
            ...
            if i > 0:
                ...
                rimg = img
            box, landmarks = self.detect_faces(
                rimg, min_face_size=sw / 10, thresholds=[0.6, 0.7, 0.7]
            )
            landmarks /= scale
            if len(landmarks) == 0:
                continue
            if i == 0:
                ...
                f5p = [
                    [w - 1 - landmarks[0][j], h - 1 - landmarks[0][j + 5]]
                    for j in range(5)
                ]
            elif i == 3:
    ...
    def align_multi(self, img, limit=None, min_face_size=64.0, crop_size=(112, 112)):
        ...
        for landmark in landmarks:
            ...
            warped_face = warp_and_crop_face(
                np.array(img), facial5points, self.refrence, crop_size=crop_size
            )
    ...
    def get_landmarks(
        ...
        for i in ori:
            if i > 0:
                ...
                rimg = img
            box, landmarks = self.detect_faces(
                rimg, min_face_size=min_face_size, thresholds=[0.6, 0.7, 0.7]
            )
            landmarks /= scale
            if len(landmarks) == 0:
                continue
